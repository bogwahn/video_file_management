...
def test_read_basic_metadata_prefers_chapter_start_time_over_start_ticks(monkeypatch, tmp_path: Path) -> None:
    video = tmp_path / "sample.mp4"
    video.write_text("")
    ...
    monkeypatch.setattr(
        tag_reader,
        "read_finder_tags",
        lambda _: SimpleNamespace(tags=[], errors=[]),
    )

    meta = mr.read_basic_metadata(video)

    assert meta.chapters
    assert meta.chapters[0].start_seconds == 2429.15
