...
def ensure_test_data() -> None:
    ...
    if os.environ.get("RUN_LARGE_INTEGRATION") != "1":
        pytest.skip("Set RUN_LARGE_INTEGRATION=1 to run large-file remux integration tests.")
...
@pytest.fixture
def mkv_clone(request: Any, tmp_path_factory: Any) -> Generator[Path, None, None]:
    ...
    if not mkv_target.exists():
        try:
            ...
            pytest.skip(f"Source media not valid for remux integration: {mp4_source} ({stderr.strip()})")
