...
| Path | Role |
|------|------|
| `config/zephyr.sample.yaml` | `hot_folder`, `dest_folder.vr`, `dest_folder.non_vr` (`Studios/{Studio}`), quarantine, collision, sidecar defaults |
...
| Key | Role | Spike default |
|-----|------|---------------|
| `hot_folder` | Initial download inbox | `/Internal/Zetc/Download` |
| `dest_folder.vr` | Final VR root | `/Volumes/Zetc/VR/{First.Actress}` |
| `dest_folder.non_vr` | Final Non-VR root | `/Volumes/ZetcOld/Studios/{Studio}` |

`{Studio}` folder spelling = **compact studio token** from the filename (e.g. `NewSensations`), same as the filename grammar. `/Volumes/ZetcOld/Uncategorized` is **superseded** as Non-VR dest.

## Naming (locked 2026-09-12)

- **Studio:** compact token — no whitespace, no internal dots. `"New Sensations"` → `NewSensations` (appears as `.NewSensations.` between actor and title blocks; Non-VR folder is also `NewSensations`).
